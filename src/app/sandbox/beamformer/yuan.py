...
def make_plots(nphones, \
    ...
    if True:
        power = sum(abs(np.fft.fft(phone_data, samples, 0)) ** 2 / samples ** 2, 0)
        plt.plot(power,'-*')
    ...
    if True:
        power = sum(abs(np.fft.fft(bf_data, samples, 0)) ** 2 / samples ** 2, 0)*2.
        # plt.plot(look_dirs * 180 / np.pi, sum(abs(np.fft.fft(bf_data, samples, 0)) ** 2 / samples ** 2, 0), '-*')
        plt.plot(look_dirs * 180 / np.pi, power, '-*')
...
if __name__ == "__main__":
    # a = [180, 60, 90]
    a = [45]
    directions = np.array(a) * np.pi / 180  # where the signal will come from
    for ind, signal_dir in enumerate(directions):
        ...
        noise_pwr = 0.1  # background noise power is .1
        ...
        # make our phone data
        phone_data, time = make_phone_data(nphones, \
                                           sound_speed, \
                                           spacing, \
                                           noise_pwr,
                                           signal_pwr,
