...
from util import __ir2, grad, grad2, grad3
...
def fill2(test_image, cluster, default=None):
    ...
    y_predict = model.predict(X.reshape(-1, 1)).astype(int)
...
def validate_clusters_func(labelled_cluster):
    ...
    if r_value > -.98:
        ...
        if len(candidate) < 1:
            log.debug("Candidate {}, dropped since density is not high enough ({})".format(g, len(candidate)))
        else:
