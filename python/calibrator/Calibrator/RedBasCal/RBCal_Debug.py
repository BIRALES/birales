...
class RBCal2:
    ...
    def calib(self, filename):
        ...
        times = len(data[:,0,0,0])
        times = int( 200 * round( times / 200. ))
        timesteps = np.linspace(0, times-200, ((times-200)/200)+1)
        ...
        for ts in timesteps:
            ...
            for t in range(ts, ts+200):
                ...
                #text_file = open("TestInteg.txt", "w")
                #for i in range(len(basl0_g)):
                #    text_file.write(str(basl0_g[i]) + '\n')
                #text_file.close()
