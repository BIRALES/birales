        ...
        if self.configuration['station']['initialise'] and self.properly_formed_station:
            ...
            if self.configuration["station"]["ada_gain"] is not None:
                ...
                if type(self.configuration["station"]["ada_gain"]) == int:
                    ...
                    logging.warning(
                        "Provided ada gain file is invalid ({})".format(self.configuration["station"]["ada_gain"]))
                else:
                    # Try loading file
    ...
    def equalize_ada_gain(self, required_rms=20):
        ...
        for tt, tile in enumerate(self.tiles):
            ...
            for channel in range(len(rms)):
                ...
                if gain < 0:
                    logging.warning(
                        "Not possible to set ADA gain lower than -6 dB for tile %d signal %d" % (tt, channel))
                elif gain > 21:
                    logging.warning(
                        "Not possible to set ADA gain higher than 15 dB for tile %d signal %d" % (tt, channel))
                    gain = 21
                else:
